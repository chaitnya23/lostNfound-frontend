  ...
  return (
    <div className="flex justify-center items-center">
      <div className="feedback-form w-[50%] border shadow-xl bg-gray-100 mt-16 rounded p-3 py-8">
        <p className="font-semibold text-center my-2 text-2xl text-[#30437b]">
          Rate us
        </p>
          ...
          {icons.map((ele, idx) => {
            return (
              <img
                src={ele}
                key={idx}
                onClick={() => handleIconChange(idx)}
                className={`cursor-pointer rounded w-16 h-16 ${
                  ratingIcon === idx ? "border-4 border-red-600" : "" 
                }`}
                alt=""
              />
            );
        ...
        <div>
          ...
          <textarea
            onChange={(e) =>
              setfeedbackInfo({ ...feedbackInfo, context: e.target.value })
            }
            value={feedbackInfo.context}
            className="w-full border-2 my-3 outline-blue-700"
            name="feedback"
            id=""
            rows="6"
          ></textarea>
        ...
        <div className="flex justify-between">
          <button
            ...
            className="w-[30%] font-xl rounded-md py-2 bg-[#30437b] text-white "
          >
            Send
          </button>
          <button
            onClick={() => navigate("/lost-items")}
            className="w-[30%] font-xl rounded-md py-2 bg-red-600 text-white "
          >
            Cancel
          </button>
        </div>
